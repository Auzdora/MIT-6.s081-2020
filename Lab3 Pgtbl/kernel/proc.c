...
found:
  ...
  // Make sure that each process's kernel page table has
  // a mapping for that process's kernel stack
...
userinit(void)
{
  ...
  uvmcopy2kvm(p->pagetable, p->kpagetable, 0, PGSIZE);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    if(uvmcopy2kvm(p->pagetable, p->kpagetable, p->sz, sz) < 0){
      return -1;
    }
...
fork(void)
{
  ...
  if(uvmcopy2kvm(np->pagetable, np->kpagetable, 0, np->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
