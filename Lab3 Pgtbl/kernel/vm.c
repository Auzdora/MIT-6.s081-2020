...
// recursive vmprint
int
recurprint(pagetable_t pagetable, uint8 depth)
{
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
...
// print vm information as requested page table pagetable
int
vmprint(pagetable_t pagetable)
{
  printf("page table %p\n", pagetable);
  int depth = 0;
  recurprint(pagetable, depth);
  return 0;
}

int
uvmcopy2kvm(pagetable_t upagetable, pagetable_t kpagetable, uint64 oldsz, uint64 newsz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = 0; i < newsz; i += PGSIZE){
    ...
    flags = PTE_FLAGS(*pte);
    flags &= (~PTE_U);
    if(kmappages(kpagetable, i, PGSIZE, (uint64)pa, flags) != 0){
      goto err;
    }
  }
  return 0;
